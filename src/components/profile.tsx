...
export default function Profile({ 
  ...
  return (
    <div onClick={func ? () => func(prev => !prev) : undefined} className='flex items-center cursor-pointer hover:opacity-90 transition-opacity p-2'>
      ...
      <p className={className ? className : 'font-bold text-xs text-nowrap ml-6 w-[50px] sm:w-[80px]'}>{name}</p>
      {func ? <MdArrowDropDown className={`${menuOpen ? 'rotate-180' : ''} transition-all`} size={24} /> : null}
    </div>
  )
}
